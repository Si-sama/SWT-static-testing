    ...
    throws ServletException, IOException {
        ...
        User_Info user = (User_Info) request.getSession().getAttribute("account");
          ...
          if(ListProduct.getInstance().getproductList().isEmpty()) { 
                  request.setAttribute("msg", "Fail , Nothing in your stock");
                     doGet(request, response);
                     return;
          }
          for (Product i : ListProduct.getInstance().getproductList()) {              
            if(Instance.getProductByID(i.getProductID()).getStatusProduct()==1) {
                request.setAttribute("msg", "Fail ,"+i.getProductName()+" has been deleted by admin, you need to remove it from "
                        + "Cart to continue");
                doGet(request, response);
                return;
            }
           ...
           boolean a=  Instance.InsertIntoOrder(user.getUserID()
              ...
              if(a==true) {
                  for (Product p :ListProduct.getInstance().getproductList() ) {
                      Instance.InsertIntoOrderDetails(Instance.getMaxOrderID(),p.getProductID() ,
                              p.getQuantity());
                  }
              }else{
...
//         }
        ...
        request.setAttribute("ThankYou", "Thank You for Buying");
        doGet(request, response);
